...
import { 
  ...
  Users, ShieldAlert, CheckSquare, XSquare, Printer, UserPlus, ShieldCheck
} from 'lucide-react';
...
export default function App() {
  ...
  // ==========================================
  // EFFECT: SINKRONISASI DATABASE (SHARED/PUBLIC)
  // ==========================================
  useEffect(() => {
    if (!user) {
       setIsDbReady(true);
    ...
    // Path diubah ke PUBLIC agar Superadmin & Admin bisa edit data yang sama
    ...
    const unsubProfile = onSnapshot(collection(db, 'artifacts', appId, 'public', 'data', 'users'), (snap) => {
      snap.forEach(d => {
    ...
    // Ambil Daftar Semua User (Untuk Manajemen User)
  ...
  const handleRegisterSubmit = async (e) => {
    ...
    try {
      ...
      // Simpan metadata user ke Firestore agar bisa dikelola Superadmin
      ...
      alert("Gagal mendaftar. Email mungkin sudah digunakan atau password terlalu pendek.");
    }
  };
  ...
  const handleEditSelect = (company) => {
    // Hanya Superadmin dan Admin yang bisa edit
    if (userProfile?.role !== 'Superadmin' && userProfile?.role !== 'Admin') {
      setDownloadToast('Maaf, hanya Admin yang dapat mengedit data.');
      setTimeout(() => setDownloadToast(''), 3000);
      return;
    }
  ...
  const handleAddCompany = () => {
    if (userProfile?.role !== 'Superadmin' && userProfile?.role !== 'Admin') {
      setDownloadToast('Maaf, hanya Admin yang dapat menambah data.');
      setTimeout(() => setDownloadToast(''), 3000);
      return;
    }
          ...
          <div className="flex items-center gap-4">
             ...
             <div className="flex items-center gap-3 pl-4 border-l border-gray-200 cursor-pointer group relative">
               <div className="hidden md:flex flex-col text-right">
                 ...
                 <span className="text-[10px] font-bold text-green-600 uppercase tracking-wider mt-1 flex items-center gap-1 justify-end"><ShieldAlert className="w-3 h-3"/> {userProfile?.role || 'User'}</span>
               </div>
               ...
               <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-6 mb-2 flex flex-col md:flex-row gap-4 items-center justify-between shrink-0">
                  <div className="flex items-center gap-3">
                     ...
                     <div>
                       ...
                       <p className="text-xs text-gray-500">Setujui permintaan akun dan atur peran sebagai Admin (Editor) atau User (Viewer).</p>
                     </div>
                  </div>
                           ...
                           {usersData.map((usr) => (
                              <tr key={usr.id} className="hover:bg-gray-50 transition-colors">
                                 <td className="px-6 py-4">
                                    ...
                                    <p className="text-[12px] text-gray-500 mt-0.5">{usr.email} • {usr.instance}</p>
                                 </td>
                                 <td className="px-6 py-4 text-center">
                                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded border uppercase ${getStatusColor(usr.role)}`}>
                                       {usr.role}
                                    </span>
                                 </td>
                                 <td className="px-6 py-4 text-center">
                                    <span className={`px-2.5 py-1 rounded text-[11px] font-bold border uppercase ${getStatusColor(usr.status)}`}>
                                       {usr.status === 'Pending' ? 'Menunggu' : 'Aktif'}
                                    </span>
                                 </td>
                                 <td className="px-6 py-4 text-right">
                                    {userProfile?.role === 'Superadmin' ? (
                                      <div className="flex justify-end gap-2">
                                         {usr.status === 'Pending' ? (
                                            <button onClick={() => handleApproveUser(usr.id)} className="flex items-center gap-1 bg-green-50 text-green-700 hover:bg-green-600 hover:text-white border border-green-200 px-3 py-1.5 rounded text-[11px] font-bold transition-colors">
                                               <CheckSquare className="w-3.5 h-3.5" /> Setujui
                                            </button>
                                         ) : (
                                            <>
                                              {usr.role !== 'Superadmin' && (
                                                <button onClick={() => handleToggleRole(usr.id, usr.role)} className="flex items-center gap-1 bg-blue-50 text-blue-700 hover:bg-blue-600 hover:text-white border border-blue-200 px-3 py-1.5 rounded text-[11px] font-bold transition-colors">
                                                   <ShieldCheck className="w-3.5 h-3.5" /> {usr.role === 'Admin' ? 'Jadikan User' : 'Jadikan Admin'}
                                                </button>
                                              )}
                                            </>
                                         )}
                                         {usr.role !== 'Superadmin' && (
                                           <button onClick={() => handleRejectUser(usr.id)} className="flex items-center gap-1 bg-red-50 text-red-700 hover:bg-red-600 hover:text-white border border-red-200 px-3 py-1.5 rounded text-[11px] font-bold transition-colors">
                                              <XSquare className="w-3.5 h-3.5" /> Hapus
                                           </button>
                                         )}
                                      </div>
                                    ) : (
            ...
            <div className="space-y-6 pb-10 animate-in fade-in duration-500">
              ...
              <div className="bg-white rounded-lg border border-gray-200 shadow p-6 overflow-hidden">
                <div className="flex items-center gap-2 mb-6 border-b border-gray-100 pb-4">
                   ...
                   <div>
                     ...
                     <p className="text-xs text-gray-500">Persentase penyelesaian RKP, Tanam, dan Serah Terima.</p>
                   </div>
                </div>
              ...
              <div>
                <div className="flex items-center gap-2 mb-5 px-1">
                   ...
                   <div>
                     <h3 className="font-bold text-gray-900 tracking-tight text-lg">Status Kepatuhan</h3>
                   </div>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-5 gap-6">
                  <div onClick={() => setSelectedDashboardStatus('Semua')} className="bg-white p-5 rounded-lg border shadow-sm border-t-4 border-t-gray-500 cursor-pointer hover:shadow-md">
                    <p className="text-[11px] font-bold text-gray-500 uppercase mb-2">Total Unit</p>
                    <p className="text-3xl font-bold text-gray-900">{currentStats.total}</p>
                  </div>
                  <div onClick={() => setSelectedDashboardStatus('Tertib')} className="bg-white p-5 rounded-lg border shadow-sm border-t-4 border-t-green-500 cursor-pointer hover:shadow-md">
                  ...
                  <div onClick={() => setSelectedDashboardStatus('SP1')} className="bg-white p-5 rounded-lg border shadow-sm border-t-4 border-t-yellow-500 cursor-pointer hover:shadow-md">
                    <p className="text-[11px] font-bold text-yellow-700 uppercase mb-2">SP1</p>
                    <p className="text-3xl font-bold text-gray-900">{currentStats.sp1}</p>
                  </div>
                  <div onClick={() => setSelectedDashboardStatus('SP2')} className="bg-white p-5 rounded-lg border shadow-sm border-t-4 border-t-orange-500 cursor-pointer hover:shadow-md">
                    <p className="text-[11px] font-bold text-orange-700 uppercase mb-2">SP2</p>
                    <p className="text-3xl font-bold text-gray-900">{currentStats.sp2}</p>
                  </div>
                  <div onClick={() => setSelectedDashboardStatus('SP3')} className="bg-white p-5 rounded-lg border shadow-sm border-t-4 border-t-red-600 cursor-pointer hover:shadow-md">
                    <p className="text-[11px] font-bold text-red-700 uppercase mb-2">SP3</p>
                  ...
                  <div className="bg-white rounded-lg border border-gray-200 shadow-md p-6 mt-6 animate-in slide-in-from-bottom-4 duration-500">
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 border-b border-gray-100 pb-4">
                      <div>
                        <h3 className="font-bold text-gray-900 tracking-tight text-lg flex items-center gap-2">
                          ...
                          Daftar Perusahaan: {selectedDashboardStatus}
                        </h3>
                      </div>
                      <div className="flex items-center gap-3">
                        <button onClick={printDashboardStatus} className="px-4 py-2 bg-gray-100 text-gray-700 hover:bg-gray-200 font-bold rounded-md text-[12px] flex items-center gap-2 border border-gray-300">
                          <Printer className="w-4 h-4" /> PDF
                        </button>
                        <button onClick={() => setSelectedDashboardStatus(null)} className="p-2 text-gray-400 hover:text-red-500 rounded"><X className="w-5 h-5" /></button>
                      </div>
                    </div>
                    ...
                    <div className="overflow-auto w-full border border-gray-200 rounded-lg max-h-[400px]">
                      <table className="w-full text-left whitespace-nowrap text-[13px]">
                        ...
                        <tbody className="divide-y divide-gray-100">
                          {dashboardDetailCompanies.map((c) => {
                            return (
                              <tr key={c.id} className="hover:bg-gray-50 transition-colors">
                                <td className="px-4 py-3 font-bold text-gray-900">{c.name}</td>
                                <td className="px-4 py-3 text-center"><span className={`text-[10px] font-bold px-2 py-0.5 rounded border uppercase ${c.category === 'PPKH' ? 'bg-amber-100 text-amber-800' : 'bg-green-100 text-green-800'}`}>{c.category}</span></td>
                                <td className="px-4 py-3 text-right font-bold text-gray-800">{c.luas || 0}</td>
                                <td className="px-4 py-3 text-center"><span className={`px-2.5 py-1 rounded text-[10px] font-bold border uppercase ${getStatusColor(c.status)}`}>{c.status}</span></td>
                              </tr>
                            );
                          })}
                        </tbody>
               ...
               <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-4 sticky top-0 z-20 flex flex-col md:flex-row gap-4 items-center justify-between">
                  ...
                  <div className="flex gap-3 w-full md:w-auto">
                    <div className="relative w-full md:w-64">
                      ...
                      <input type="text" placeholder="Cari unit perusahaan..." className="w-full pl-10 pr-4 py-2 text-[13px] bg-gray-50 border border-gray-300 rounded-md focus:ring-1 focus:ring-green-600 outline-none" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                    </div>
                  </div>
               </div>
              ...
              <div className={`bg-white rounded-lg border border-gray-200 shadow flex flex-col transition-all duration-500 ease-in-out ${selectedCompany ? 'h-[300px] shrink-0' : 'flex-1 min-h-[500px]'}`}>
                <div className="p-4 border-b border-gray-200 flex flex-col gap-4 bg-gray-50 shrink-0 rounded-t-lg">
                  <div className="flex flex-col xl:flex-row justify-between xl:items-center gap-4">
                    ...
                    <div className="flex bg-white p-1 rounded-md border border-gray-300 w-fit shadow-sm">
                      <button onClick={() => setFilterCategory('Semua')} className={`px-5 py-1.5 rounded-md font-semibold transition-all text-[13px] ${filterCategory === 'Semua' ? 'bg-gray-700 text-white shadow' : 'text-gray-600 hover:bg-gray-100'}`}>Semua Data</button>
                      <button onClick={() => setFilterCategory('PPKH')} className={`px-5 py-1.5 rounded-md font-semibold transition-all flex items-center gap-2 text-[13px] ${filterCategory === 'PPKH' ? 'bg-amber-600 text-white shadow' : 'text-gray-600 hover:bg-gray-100'}`}><Mountain className="w-4 h-4" /> PPKH</button>
                      <button onClick={() => setFilterCategory('PKTMKH')} className={`px-5 py-1.5 rounded-md font-semibold transition-all flex items-center gap-2 text-[13px] ${filterCategory === 'PKTMKH' ? 'bg-green-700 text-white shadow' : 'text-gray-600 hover:bg-gray-100'}`}><Leaf className="w-4 h-4" /> PKTMKH</button>
                    </div>
                    
                    <div className="flex items-center gap-3 w-full xl:w-auto">
                      <div className="relative w-full sm:w-64">
                        <Search className="w-4 h-4 absolute left-3 top-3 text-gray-400" />
                        <input type="text" placeholder="Cari unit perusahaan..." className="w-full pl-10 pr-4 py-2 text-[14px] bg-white border border-gray-300 rounded-md focus:ring-1 focus:ring-green-600 outline-none" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                      </div>
                        ...
                        return tasks.map((taskData, idx) => {
                          ...
                          return (
                            <tr key={`${c.id}-${idx}`} onClick={() => handleEditSelect(c)} className="hover:bg-green-50 cursor-pointer transition-colors">
                              ...
                              <td className="px-6 py-4 text-center"><span className={`px-2.5 py-1 rounded text-[11px] font-bold border uppercase ${getStatusColor(currentStatus)}`}>{currentStatus}</span></td>
              ...
              {selectedCompany && editFormData && (
                <div className="bg-white rounded-lg border border-gray-200 shadow-md flex flex-col shrink-0 animate-in slide-in-from-bottom-4 duration-500">
                  <div className="p-4 md:p-6 border-b border-gray-200 bg-gray-50 flex justify-between items-center shrink-0 rounded-t-lg">
                    <div>
                      <h3 className="font-bold text-gray-900 text-lg flex items-center gap-2">
                        ...
                        Formulir Manajemen Data
                      </h3>
                      <p className="text-sm text-gray-500 mt-1">Mengedit data unit: <strong className="text-green-700">{editFormData.company.name || 'Baru'}</strong></p>
                    </div>
                    <div className="flex items-center gap-4">
                      {showSaveSuccess && (
                        <span className="bg-green-100 text-green-800 px-4 py-2 rounded-md text-[13px] font-bold border border-green-300">
                          <CheckCircle className="w-4 h-4 inline mr-2" /> Disimpan ke Database Bersama
                        </span>
                      )}
                      <button onClick={() => { setSelectedCompany(null); setEditFormData(null); }} className="p-2 text-gray-500 hover:text-red-600 rounded-md transition-colors"><X className="w-5 h-5" /></button>
                    </div>
                  </div>

                  <div className="p-6 md:p-8 space-y-8 bg-white">
                    <section>
                      <h4 className="text-[13px] font-bold text-gray-500 uppercase tracking-wide mb-4 border-b border-gray-200 pb-2">Identitas Unit</h4>
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                        <div className="md:col-span-2"><label className="block text-xs font-bold text-gray-600 mb-1.5">Nama Perusahaan</label><input type="text" className="w-full px-4 py-2 border border-gray-300 rounded-md text-[14px]" value={editFormData.company.name} onChange={(e) => handleCompanyChange('name', e.target.value)} /></div>
                        <div className="md:col-span-1"><label className="block text-xs font-bold text-gray-600 mb-1.5">Sektor</label><input type="text" className="w-full px-4 py-2 border border-gray-300 rounded-md text-[14px]" value={editFormData.company.sector || ''} onChange={(e) => handleCompanyChange('sector', e.target.value)} /></div>
                        <div className="md:col-span-1"><label className="block text-xs font-bold text-gray-600 mb-1.5">Kategori</label><select className="w-full px-4 py-2 border border-gray-300 rounded-md text-[14px]" value={editFormData.company.category} onChange={(e) => handleCompanyChange('category', e.target.value)}><option value="PPKH">PPKH</option><option value="PKTMKH">PKTMKH</option></select></div>
                      </div>
                    </section>

                    <section>
                      <h4 className="text-[13px] font-bold text-gray-500 uppercase tracking-wide mb-4 border-b border-gray-200 pb-2">Manajemen Kewajiban</h4>
                        ...
                        return (
                          <div key={task.id} className="bg-gray-50 border border-gray-200 rounded-lg p-6 mb-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                               <div><label className="block text-[11px] font-bold text-gray-600 uppercase tracking-wide mb-1.5">No. SK Penetapan</label><input type="text" className="w-full px-4 py-2 border border-gray-300 rounded-md" value={task.sk_lokasi} onChange={(e) => handleTaskChange(index, 'sk_lokasi', e.target.value)} /></div>
                               <div><label className="block text-[11px] font-bold text-gray-600 uppercase tracking-wide mb-1.5">Luas SK (Ha)</label><input type="number" step="any" className="w-full px-4 py-2 border border-gray-300 rounded-md" value={task.luas || ''} onChange={(e) => handleTaskChange(index, 'luas', e.target.value)} /></div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                               <div className="bg-white p-5 rounded-lg border border-gray-200">
                                 ...
                                 {(task.riwayat_rkp || []).map((r, histIndex) => (
                                   <div key={histIndex} className="flex gap-2 mb-2">
                                      <input type="number" className="w-20 px-2 py-1 border border-gray-300 rounded text-[12px]" value={r.tahun} onChange={(e) => updateHistory(index, 'riwayat_rkp', histIndex, 'tahun', e.target.value)} />
                                      <input type="number" className="flex-1 px-2 py-1 border border-gray-300 rounded text-[12px]" value={r.luas} onChange={(e) => updateHistory(index, 'riwayat_rkp', histIndex, 'luas', e.target.value)} />
                                   </div>
                                 ))}
                                 <button type="button" onClick={() => addHistory(index, 'riwayat_rkp')} className="text-[11px] text-blue-600 font-bold">+ Tambah</button>
                               </div>

                               <div className="bg-white p-5 rounded-lg border border-gray-200">
                                 <label className="text-[12px] font-bold text-gray-800 uppercase mb-4 block">Riwayat Tanam</label>
                                 {(task.riwayat_tanam || []).map((r, histIndex) => (
                                   <div key={histIndex} className="flex gap-2 mb-2">
                                      <input type="number" className="w-16 px-2 py-1 border border-gray-300 rounded text-[12px]" value={r.tahun} onChange={(e) => updateHistory(index, 'riwayat_tanam', histIndex, 'tahun', e.target.value)} />
                                      <input type="number" className="flex-1 px-2 py-1 border border-gray-300 rounded text-[12px]" value={r.luas} onChange={(e) => updateHistory(index, 'riwayat_tanam', histIndex, 'luas', e.target.value)} />
                                   </div>
                                 ))}
                                 <button type="button" onClick={() => addHistory(index, 'riwayat_tanam')} className="text-[11px] text-green-600 font-bold">+ Tambah</button>
                               </div>

                               <div className="bg-white p-5 rounded-lg border border-gray-200">
                                 <label className="text-[12px] font-bold text-gray-800 uppercase mb-4 block">Riwayat Serah Terima</label>
                                 {(task.riwayat_serah_terima || []).map((r, histIndex) => (
                                   <div key={histIndex} className="flex gap-2 mb-2">
                                      <input type="number" className="w-16 px-2 py-1 border border-gray-300 rounded text-[12px]" value={r.tahun} onChange={(e) => updateHistory(index, 'riwayat_serah_terima', histIndex, 'tahun', e.target.value)} />
                                      <input type="number" className="flex-1 px-2 py-1 border border-gray-300 rounded text-[12px]" value={r.luas} onChange={(e) => updateHistory(index, 'riwayat_serah_terima', histIndex, 'luas', e.target.value)} />
                                   </div>
                                 ))}
                                 <button type="button" onClick={() => addHistory(index, 'riwayat_serah_terima')} className="text-[11px] text-orange-600 font-bold">+ Tambah</button>
                               </div>
                            </div>
                          </div>
                        );
                  ...
                  <div className="p-6 md:p-8 border-t border-gray-200 bg-gray-50 flex justify-end gap-4 rounded-b-lg">
                    <button type="button" onClick={() => { setSelectedCompany(null); setEditFormData(null); }} className="px-6 py-2 bg-white border border-gray-300 rounded-md">Batal</button>
                    <button type="button" onClick={handleSaveChanges} className="px-8 py-2 bg-green-700 text-white font-bold rounded-md flex items-center gap-2">
